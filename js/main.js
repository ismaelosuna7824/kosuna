...
function cargaDatosSuccess(tx, results){
	...
	for(var i=0; i<results.rows.length; i++){
		...
		selector.append('<li id="li_'+persona.id+'"><a href="#detalle" data-uid='+persona.id+' class="linkDetalles"><div class="interior_lista"><img src="'+ foto +'" class="img_peq"/><span>' + persona.nombre + ' ' + persona.domicilio+ '</span></div></a><a href="#form"  data-theme="a" data-uid='+persona.id+'  class="linkForm">Predet.</a></li>').listview('refresh');
	}
...
function queryFormSuccess(tx, results) {
	...
	$.registro = results.rows.item(0);
		...
		$("#txt_nombre").val($.registro.nombre);
		$("#txt_domicilio").val($.registro.domicilio);
		$("#txt_telefono").val($.registro.telefono);
		$("#txt_mail").val($.registro.email);
		$("#txt_nota").val($.registro.nota);
...
function initForm(){
	...
	$("#txt_amigos").trigger("click").trigger("click")
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE actividades SET nombre="'+$("#txt_nombre").val()+'", domicilio="'+$("#txt_domicilio").val()+'",telefono="'+$("#txt_telefono").val()+'",email="'+$("#txt_mail").val()+'",nota="'+$("#txt_nota").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
function updateFormSuccess(tx) {
	...
	selector.find("a:first").find("span").html($("#txt_nombre").val() + " " + $("#txt_domicilio").val());
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO actividades (nombre,domicilio,telefono,categoria,foto,email, nota) VALUES ('"+$("#txt_nombre").val()+"','"+$("#txt_domicilio").val()+"','"+$("#txt_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#txt_mail").val()+"','"+$("#txt_nota").val()+"' )", [], newFormSuccess, errorDB);
}
function newFormSuccess(tx, results) {
	...
	var obj = $('<li id="li_'+results.insertId+'"><a href="#detalle" data-uid='+results.insertId+' class="linkDetalles"><div class="interior_lista"><img src="'+ $.imageUR +'" class="img_peq"/><span>' + $("#txt_nombre").val() + " " + $("#txt_domicilio").val()+ '</span></div></a><a href="#form"  data-theme="a" data-uid='+results.insertId+'  class="linkForm">Predet.</a></li>');
